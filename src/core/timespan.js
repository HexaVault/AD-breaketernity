window.TimeSpan = class TimeSpan {
  ...
  toStringNoDecimals(bypassEND = false) {
    ...
    function addComponent(value, name) {
      parts.push(value.eq(1) ? `${formatInt(value, bypassEND)} ${name}` : `${formatInt(value, bypassEND)} ${name}s`);
    }
  ...
  toStringShort(useHMS = true, bypassEND = false) {
    ...
    if (totalSeconds.gt(5e-7) && totalSeconds.lt(1e-3)) {
      ...
      return `${format(totalSeconds.times(1000), 0, 3, bypassEND)} ms`;
    }
    if (totalSeconds.lt(1)) {
      ...
      return `${format(totalSeconds.times(1000), 0, 0, bypassEND)} ms`;
    }
