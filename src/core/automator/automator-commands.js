...
export const AutomatorCommands = [
  ...
  },
  {
    ...
    compile: ctx => {
      ...
      return () => {
        if (player.options.notifications.automator) GameUI.notify.automator(`Automator: ${notifyText[0].image}`);
    ...
    compile: ctx => {
      ...
      return S => {
        ...
        if (S.commandState === null) {
          ...
          S.commandState.timeMs += Decimal.max(Time.trueDeltaTime.totalMilliseconds,
            AutomatorBackend.currentInterval).toNumber();
        }
