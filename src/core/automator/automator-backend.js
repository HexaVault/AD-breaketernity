...
export const AutomatorData = {
  ...
  MAX_ALLOWED_VARIABLE_COUNT: 12,
  MAX_ALLOWED_VARIABLE_NAME_LENGTH: 25,
  ...
  createNewScript(content, name) {
    ...
    if (player.options.notifications.automator) GameUI.notify.automator(`Imported Script "${name}"`);
  ...
  */
  ...
  step() {
    ...
    if (!this.hasJustCompleted) {
      if (player.options.notifications.automator) GameUI.notify.error("Automator halted - too many consecutive no-ops detected");
