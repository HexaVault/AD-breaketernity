  ...
  methods: {
    ...
    updateChallengePower() {
      ...
      if (isChallengePowerVisible) {
        const powerArray = [];
        if (isC2Running) powerArray.push(`Production: ${formatPercents(player.challengeData.nc2percent, 2, 2)}`);
        if (isC3Running) powerArray.push(`First dimension: ${formatX(player.challengeData.nc3pow, 3, 4)}`);
