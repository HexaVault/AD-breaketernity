...
export default {
  ...
  methods: {
    update() {
      ...
      this.singularityCap = new Decimal(Singularity.cap);
      this.singularityWaitTime = TimeSpan.fromSeconds((this.singularityCap.sub(this.darkEnergy)).div(Currency.darkEnergy.productionPerSecond)).toStringShort();
